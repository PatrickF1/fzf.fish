...
function fzf_configure_bindings --description "Installs the default key bindings for fzf.fish with user overrides passed as options."
    ...
    set -f options_spec h/help 'directory=?' 'git_log=?' 'git_status=?' 'grep=?' 'history=?' 'processes=?' 'variables=?'
    ...
    if test $status -ne 0
        ...
        # index 1 = directory, 2 = git_log, 3 = git_status, 4 = grep, 5 = history, 6 = processes, 7 = variables
        set -f key_sequences \e\cf \e\cl \e\cs \e\cg \cr \e\cp \cv # \c = control, \e = escape
        set --query _flag_directory && set key_sequences[1] "$_flag_directory"
        set --query _flag_git_log && set key_sequences[2] "$_flag_git_log"
        set --query _flag_git_status && set key_sequences[3] "$_flag_git_status"
        set --query _flag_grep && set key_sequences[4] "$_flag_grep"
        ...
        for mode in default insert
            ...
            test -n $key_sequences[4] && bind --mode $mode $key_sequences[4] _fzf_search_grep
